...
var (
	agentLogPath = defaultServiceLogPath("edge-agent", "agent.log")
...
func logObservability(category, component, event, status string, context map[string]any, console string) {
	...
	if err := appendJSONLog(agentLogPath, entry, &agentLogMu); err != nil {
		log.Printf("[LOGGER][ERROR] path=%s err=%v", agentLogPath, err)
	}
	if console != "" {
		log.Print(console)
	}
